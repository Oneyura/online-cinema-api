...
class MovieCreate(BaseModel):
    uuid: UUID = Field(
        ..., example="123e4567-e89b-12d3-a456-426614174000", description="Unique identifier for the movie"
    )
    name: str = Field(..., min_length=1, max_length=255, example="Inception", description="Movie title")
    year: int = Field(..., gt=1800, lt=date.today().year + 5, example=2010, description="Release year")
    time: int = Field(..., gt=0, example=148, description="Duration in minutes")
    imdb: float = Field(..., ge=0.0, le=10.0, example=8.8, description="IMDb rating (0.0 to 10.0)")
    votes: int = Field(..., ge=0, example=2400000, description="Number of IMDb votes")
    meta_score: Optional[float] = Field(None, ge=0.0, le=100.0, example=74.0, description="Metascore (0.0 to 100.0)")
    gross: Optional[float] = Field(None, ge=0.0, example=292.6, description="Gross revenue (in millions)")
    description: str = Field(
        ...,
        min_length=10,
        example="A thief who steals corporate secrets " "through use of dream-sharing technology...",
        description="Movie synopsis",
    )
    price: condecimal(max_digits=10, decimal_places=2) = Field(
        ..., gt=0, example=9.99, description="Price of the movie"
    )
    certification_id: int = Field(..., example=1, description="ID of the movie's certification (e.g., PG-13, R)")

    genre_ids: Optional[List[int]] = Field(None, description="List of Genre IDs associated with the movie")
    director_ids: Optional[List[int]] = Field(None, description="List of Director IDs associated with the movie")
    star_ids: Optional[List[int]] = Field(None, description="List of Star (Actor) IDs associated with the movie")
...
class CommentCreate(BaseModel):
    text: str = Field(..., min_length=1, max_length=1000, example="This movie was fantastic!")
...
class MovieLikeCreate(BaseModel):
    is_liked: bool = Field(..., description="True for like, False for dislike", example=True)
...
class MovieRatingCreate(BaseModel):
    rating: int = Field(..., ge=1, le=10, description="Rating on a 1-10 scale", example=9)
