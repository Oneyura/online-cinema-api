...
class TestMovieCRUD:
    ...
    async def test_create_movie(self, moderator_client: AsyncClient, db_session: AsyncSession):
        ...
        actor_joseph = (
            (await db_session.execute(select(ActorModel).filter_by(name="Joseph Gordon-Levitt"))).scalars().first()
        )
        actor_joseph_id = actor_joseph.id
        director_nolan = (
            (await db_session.execute(select(DirectorModel).filter_by(name="Christopher Nolan"))).scalars().first()
        )
    ...
    async def test_delete_movie_with_purchase(
        self, moderator_client: AsyncClient, db_session: AsyncSession, create_test_user
    ):
        ...
        movie_purchased = MovieModel(
            uuid=uuid4(),
            name="Purchased Movie",
            year=2018,
            time=110,
            imdb=7.8,
            votes=1000,
            description="Movie that has been purchased.",
            price=Decimal("15.00"),
            certification_id=cert.id,
        )
        ...
        purchase = PurchaseModel(
            user_id=user.id, movie_id=movie_purchased.id, purchase_date=datetime.now(), price=Decimal("15.00")
        )
...
class TestMovieInteractions:
    ...
    async def test_dislike_movie(self, authenticated_client: AsyncClient, db_session: AsyncSession):
        ...
        movie = MovieModel(
            uuid=uuid4(),
            name="Dislikeable Movie",
            year=2023,
            time=105,
            imdb=6.5,
            votes=300,
            description="A movie to dislike.",
            price=Decimal("7.50"),
            certification_id=cert.id,
        )
        db_session.add(movie)
    ...
    async def test_update_like_status(self, authenticated_client: AsyncClient, db_session: AsyncSession):
        ...
        movie = MovieModel(
            uuid=uuid4(),
            name="Movie for Like Update",
            year=2024,
            time=115,
            imdb=8.0,
            votes=700,
            description="Movie to test update.",
            price=Decimal("10.00"),
            certification_id=cert.id,
        )
        db_session.add(movie)
    ...
    async def test_write_comment(
        self, authenticated_client: AsyncClient, db_session: AsyncSession, authenticated_user: UserModel
    ):
        ...
        movie = MovieModel(
            uuid=uuid4(),
            name="Commentable Movie",
            year=2020,
            time=100,
            imdb=7.0,
            votes=500,
            description="A movie for comments.",
            price=Decimal("9.00"),
            certification_id=cert.id,
        )
        db_session.add(movie)
        ...
        assert (
            response.json()["user"]["username"] == authenticated_user.username
        )  # Use the actual authenticated user's username

        comment_in_db = await db_session.execute(
            select(CommentModel).filter_by(movie_id=movie.id, user_id=authenticated_user.id)
        )
        assert comment_in_db.scalars().first() is not None

    async def test_get_movie_comments(
        self, authenticated_client: AsyncClient, db_session: AsyncSession, create_test_user
    ):
        ...
        movie = MovieModel(
            uuid=uuid4(),
            name="Movie with Comments",
            year=2021,
            time=110,
            imdb=7.5,
            votes=600,
            description="Movie to get comments for.",
            price=Decimal("11.00"),
            certification_id=cert.id,
        )
        db_session.add(movie)
        ...
        comment1 = CommentModel(
            user_id=user1.id, movie_id=movie.id, text="Great movie!", created_at=datetime(2023, 1, 1, 10, 0, 0)
        )
        comment2 = CommentModel(
            user_id=user2.id, movie_id=movie.id, text="Really enjoyed it.", created_at=datetime(2023, 1, 1, 11, 0, 0)
        )
    ...
    async def test_add_movie_to_favorites(self, authenticated_client: AsyncClient, db_session: AsyncSession):
        ...
        movie = MovieModel(
            ...
            imdb=8.5,
            votes=1000,
            description="A movie for favorites.",
            price=Decimal("12.00"),
            certification_id=cert.id,
        )
        db_session.add(movie)
    ...
    async def test_remove_movie_from_favorites(
        ...
        movie = MovieModel(
            ...
            imdb=7.0,
            votes=400,
            description="A movie to unfavorite.",
            price=Decimal("6.00"),
            certification_id=cert.id,
        )
        db_session.add(movie)
    ...
    async def test_rate_movie(
        ...
        rating_in_db = await db_session.execute(
            select(MovieRatingModel).filter_by(movie_id=movie.id, user_id=authenticated_user.id)
        )
